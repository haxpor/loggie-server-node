...
var header = "/*!\n" +
	...
	" * https://github.com/haxpor/loggie-server-node/blob/master/LICENSE.md\n" +
	" */\n";
...
function buildDevTask() {
	...
	return gulp.src('dist/loggieServer-es6.js')
		.pipe(babel({
			presets: ['es2015']
		}))
		.pipe(streamify(concat('loggieServer-dev-es5.js')))
...
function buildInitialTask() {
	...
	return browserify('./src/loggie-server-node.js', { standalone: 'loggieServer' })
		.plugin(collapse)
		.bundle()
		.pipe(source('loggieServer-es6.js'))
...
function buildNormalTask() {
	...
	return gulp.src('dist/loggieServer-es6.js')
		...
		.pipe(streamify(replace('{{ version }}', package.version)))
		.pipe(streamify(concat('loggieServer.js')))
		.pipe(gulp.dest(outDir));
}
...
function buildMinifiedTask() {
	...
	return gulp.src('dist/loggieServer-es6.js')
		...
		.pipe(streamify(replace('{{ version }}', package.version)))
		.pipe(streamify(concat('loggieServer.min.js')))
		.pipe(gulp.dest(outDir));
}
